...
int main(int argc, char * argv[])
{
  ...
  const std::string ee_frame = "arm_tool_link";
  const std::string gripper_frame = "gripper_grasping_frame";
...
RCLCPP_INFO(logger, "Table reset to its original world pose.");
  ...
  // --- PUT-DOWN PHASE: place the cube at (x0, -y0, z0)
  // Compute mirrored cube pose (x0, -y0, z0)
  geometry_msgs::msg::Pose mirror_cube_pose = cube_pose;
  mirror_cube_pose.position.y = -mirror_cube_pose.position.y;


  // Compute EE poses for pre-putdown and putdown using existing transforms
  tf2::Transform cube_mirror_in_base;
  tf2::fromMsg(mirror_cube_pose, cube_mirror_in_base);



  tf2::Transform place_eef_in_base = cube_mirror_in_base * cube_to_top_gripper * e_to_gripper_tf.inverse();
  tf2::Transform preplace_eef_in_base = cube_mirror_in_base * cube_to_pregrasp_gripper * e_to_gripper_tf.inverse();

  geometry_msgs::msg::Pose preplace_eef_pose;
  {
    const auto &pre_o = preplace_eef_in_base.getOrigin();
    const auto pre_q = preplace_eef_in_base.getRotation();
    preplace_eef_pose.position.x = pre_o.x();
    preplace_eef_pose.position.y = pre_o.y();
    preplace_eef_pose.position.z = pre_o.z();
    preplace_eef_pose.orientation = tf2::toMsg(pre_q);
  }

  geometry_msgs::msg::Pose place_eef_pose;
  {
    const auto &o = place_eef_in_base.getOrigin();
    const auto q = place_eef_in_base.getRotation();
    // For putdown use exact contact pose (no extra pregrasp_distance offset)
    place_eef_pose.position.x = o.x();
    place_eef_pose.position.y = o.y();
    place_eef_pose.position.z = o.z()+pregrasp_distance*1.5;
    place_eef_pose.orientation = tf2::toMsg(q);
  }

  // Go to pre-putdown
  if (!plan_and_execute(preplace_eef_pose, "pozycji pre-putdown")) {
    RCLCPP_WARN(logger, "Planowanie do pozycji pre-putdown nie powiodło się. Pomijam odkładanie.");
  } else {
    // Temporarily attach the table to allow safe descent contact
    moveit_msgs::msg::AttachedCollisionObject aco_table;
    aco_table.link_name = ee_frame;
    aco_table.object.id = "table";
    aco_table.object.operation = moveit_msgs::msg::CollisionObject::ADD;
    for (const auto &ln : gripper_group.getLinkNames()) {
      aco_table.touch_links.push_back(ln);
    }
    psi.applyAttachedCollisionObject(aco_table);
    rclcpp::sleep_for(200ms);
    move_group_interface.setStartStateToCurrentState();

    // Move down to place pose
    if (!plan_and_execute(place_eef_pose, "pozycji putdown")) {
      RCLCPP_WARN(logger, "Planowanie do pozycji putdown nie powiodło się.");

      // Detach and restore table in world on failure
      move_group_interface.detachObject("table");
      rclcpp::sleep_for(100ms);
      move_group_interface.setStartStateToCurrentState();
      moveit_msgs::msg::CollisionObject table_reset_fail;
      table_reset_fail.header.frame_id = planning_frame;
      table_reset_fail.id = "table";
      table_reset_fail.primitives.resize(1);
      table_reset_fail.primitives[0].type = shape_msgs::msg::SolidPrimitive::BOX;
      table_reset_fail.primitives[0].dimensions = {1, 0.8, 0.03};
      table_reset_fail.primitive_poses.resize(1);
      table_reset_fail.primitive_poses[0] = table_pose_world;
      table_reset_fail.operation = table_reset_fail.ADD;
      psi.applyCollisionObject(table_reset_fail);
      rclcpp::sleep_for(100ms);
      RCLCPP_INFO(logger, "Przywrócono stół w scenie po nieudanym planowaniu putdown.");
    } else {
      // Open gripper to release the cube
      {
        std::map<std::string, double> open_targets = {
          {left_joint,  open_val},
          {right_joint, open_val}
        };
        gripper_group.setJointValueTarget(open_targets);
        auto ec_open = gripper_group.move();
        if (ec_open != moveit::core::MoveItErrorCode::SUCCESS) {
          RCLCPP_WARN(logger, "Nie udało się otworzyć chwytaka po odłożeniu.");
        } else {
          RCLCPP_INFO(logger, "Chwytak otwarty – zwolniono kostkę.");
        }
      }

      // // Detach cube and table, then restore them in the world
      // move_group_interface.detachObject("green_cube_3");
      // move_group_interface.detachObject("table");
      // rclcpp::sleep_for(150ms);
      // move_group_interface.setStartStateToCurrentState();

      // // Re-add table at its original pose
      // moveit_msgs::msg::CollisionObject table_reset_ok;
      // table_reset_ok.header.frame_id = planning_frame;
      // table_reset_ok.id = "table";
      // table_reset_ok.primitives.resize(1);
      // table_reset_ok.primitives[0].type = shape_msgs::msg::SolidPrimitive::BOX;
      // table_reset_ok.primitives[0].dimensions = {1, 0.8, 0.03};
      // table_reset_ok.primitive_poses.resize(1);
      // table_reset_ok.primitive_poses[0] = table_pose_world;
      // table_reset_ok.operation = table_reset_ok.ADD;
      // psi.applyCollisionObject(table_reset_ok);

      // // Re-add cube at the mirrored pose (final placed location)
      // moveit_msgs::msg::CollisionObject cube_reset;
      // cube_reset.header.frame_id = planning_frame;
      // cube_reset.id = "green_cube_3";
      // cube_reset.primitives.resize(1);
      // cube_reset.primitives[0].type = shape_msgs::msg::SolidPrimitive::BOX;
      // cube_reset.primitives[0].dimensions = {0.07, 0.07, 0.07};
      // cube_reset.primitive_poses.resize(1);
      // cube_reset.primitive_poses[0] = mirror_cube_pose;
      // cube_reset.operation = cube_reset.ADD;
      // psi.applyCollisionObject(cube_reset);
      // rclcpp::sleep_for(150ms);
      // RCLCPP_INFO(logger, "Kostka odłożona w (x0, -y0, z0) i przywrócona do sceny planowania.");
      ...
      table.primitives[0].dimensions = {1, 0.8, 0.03};
      table.primitive_poses.resize(1);
      table.primitive_poses[0] = table_pose_world;
      table.operation = table.ADD;
      objects.push_back(table);
      ...
      cube.primitives[0].dimensions = {0.07, 0.07, 0.07};
      cube.primitive_poses.resize(1);
      cube.primitive_poses[0] = mirror_cube_pose;
      cube.operation = cube.ADD;
      objects.push_back(cube);
      ...
      // // Retreat back up to pre-putdown
      // if (!plan_and_execute(preplace_eef_pose, "powrotu nad kostkę (pre-putdown)")) {
      //   RCLCPP_WARN(logger, "Planowanie powrotu nad kostkę nie powiodło się.");
      // } else {
      //   RCLCPP_INFO(logger, "Powrót nad kostkę zakończony.");
      // }
    }
  }

...
from dynamics import lv_dynamics, num_dim
import numpy as np

def get_initial_states(steering_table, x_guess=np.array([50.0, 20.0])):
    ...
    for i in range(num_trajectories):
        ...
        def fun(x):
            ...
            xdot = lv_dynamics(0.0, state_tensor, u_func)
...
def generate_initial_conditions(num_trajectories, num_dim, num_steering_jump, steps_per_jump):
    steering_table = (torch.rand((num_steering_jump, num_trajectories, 1)) * 10).repeat_interleave(steps_per_jump, dim=0)
...
def generate_trajectories(y_0s, steering_table, dt, T):
    ...
    trajectories = odeint(lambda t, state: lv_dynamics(t, state, u_func, device), y_0s, times)
